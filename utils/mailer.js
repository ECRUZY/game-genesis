  ...
  const data = JSON.stringify({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#080c12;color:#fff;border-radius:12px;overflow:hidden;">
        ...
        <div style="padding:28px;">
          <div style="font-size:16px;margin-bottom:8px;">Привет, <b>${username}</b>!</div>
          <div style="color:#7a9cc4;font-size:14px;margin-bottom:24px;">Ты запросил сброс пароля. Нажми кнопку ниже:</div>
          <a href="${resetUrl}" style="display:block;background:#f0c040;color:#080c12;text-align:center;padding:14px;border-radius:8px;font-weight:900;font-size:15px;text-decoration:none;margin-bottom:20px;">Сбросить пароль</a>
          <div style="color:#4a6a8a;font-size:12px;">Ссылка действует 1 час. Если ты не запрашивал сброс — просто проигнорируй это письмо.</div>
        </div>
        <div style="background:#0a1428;padding:14px 28px;text-align:center;">
...
module.exports = { sendVerificationCode, sendResetPassword }
